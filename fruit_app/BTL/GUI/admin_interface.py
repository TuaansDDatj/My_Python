...
class LibraryApp:
    def __init__(self, root, db, book_manager, user_manager, borrow_manager, return_manager, report_manager):
        ...
        self.root.title("ỨNG DỤNG QUẢN LÝ THƯ VIỆN ĐẠI HỌC - ADMIN")
    ...
    def print_return_slip(self, ma_phieu_tra):
        try:
            ...
            messagebox.showerror("Lỗi", f"Thiếu thư viện: {str(e)}\nHãy cài đặt python-docx bằng lệnh: pip install python-docx")
            return False
        except PermissionError:
            messagebox.showerror("Lỗi", "Không có quyền truy cập để lưu tệp. Vui lòng chọn vị trí lưu khác.")
            return False
    ...
    def create_menu_frame(self):
        ...
        admin_text_label = tk.Label(menu_frame, text="Xin chào, Admin!", font=("Arial", 12),
                                  bg="#2c3e50", fg="white", justify="center")
    ...
    def show_book_management(self):
        ...
        labels = ["Mã Sách", "Mã Thể Loại", "Tên Sách", "Tác Giả", "Thể Loại", "Số Lượng", "Nhà Xuất Bản", "Giá Trị", "Tình Trạng"]
        entries = {}
        ...
        btn_add = tk.Button(button_frame, text="Thêm", bg="#1abc9c", fg="white", font=("Arial", 10), width=10, command=add_book)
        btn_add.pack(side="left", padx=5)

        btn_edit = tk.Button(button_frame, text="Sửa", bg="#f39c12", fg="white", font=("Arial", 10), width=10, command=edit_book)
        btn_edit.pack(side="left", padx=5)

        btn_delete = tk.Button(button_frame, text="Xóa", bg="#e74c3c", fg="white", font=("Arial", 10), width=10, command=delete_book)
        btn_delete.pack(side="left", padx=5)

        btn_search = tk.Button(button_frame, text="Tìm kiếm", bg="#3498db", fg="white", font=("Arial", 10), width=10, command=search_book)
        btn_search.pack(side="left", padx=5)

        btn_clear = tk.Button(button_frame, text="Làm mới", bg="#95a5a6", fg="white", font=("Arial", 10), width=10, command=clear_entries)
        ...
        columns = labels
        book_list = ttk.Treeview(table_frame, columns=columns, show="headings", height=15)

        for col in columns:
            book_list.heading(col, text=col)
            if col in ["Mã Sách", "Mã Thể Loại", "Tác Giả"]:
                book_list.column(col, width=80, anchor="center")
            elif col in ["Số Lượng", "Giá Trị","Tình Trạng"]:
                book_list.column(col, width=50, anchor="center")
            elif col in ["Tên Sách", "Thể Loại", "Nhà Xuất Bản"]:
        ...
        # Tạo TreeView và liên kết với scrollbar
        book_list.configure(yscrollcommand=y_scrollbar.set)
        y_scrollbar.configure(command=book_list.yview)
        ...
        for i, label_text in enumerate(cat_labels):
            ...
            label = tk.Label(field_frame, text=label_text, font=("Arial", 10), bg="#f8f9fa", width=12, anchor="center")
        ...
        # Category buttons
        btn_add_cat = tk.Button(cat_button_frame, text="Thêm", bg="#1abc9c", fg="white", font=("Arial", 10), width=10, command=add_category)
        btn_add_cat.pack(side="left", padx=5)

        btn_clear_cat = tk.Button(cat_button_frame, text="Làm mới", bg="#95a5a6", fg="white", font=("Arial", 10), width=10, command=clear_cat_entries)
    ...
    def show_borrow_return_books(self):
        ...
        btn_add_borrow = tk.Button(button_frame_borrow, text="Thêm", bg="#1abc9c", fg="white", font=("Arial", 10), width=10, command=add_borrow)
        btn_add_borrow.pack(side="left", padx=5)

        btn_clear_borrow = tk.Button(button_frame_borrow, text="Làm mới", bg="#95a5a6", fg="white", font=("Arial", 10), width=10, command=clear_borrow_entries)
        ...
        def print_selected_borrow():
            ...
            print("ID phiếu mượn đã chọn:", ma_phieu_muon)
        ...
        def show_borrow_details(event):
            ...
            if selected_items:
                ...
                lbl_title = tk.Label(header_frame, text=f"Chi tiết phiếu mượn #{ma_phieu_muon}", font=("Arial", 14, "bold"), bg="#f8f9fa")
                lbl_title.pack(side="left", pady=10)

                reader_info = tk.Label(header_frame, text=f"Độc giả: {values[2]} ({values[1]})", font=("Arial", 12), bg="#f8f9fa")
        ...
        for i, field_text in enumerate(return_fields):
            ...
            if field_text == "Mã Độc Giả":
                ...
                entry = ttk.Combobox(field_frame, values=["Tốt", "Bình thường", "Hư hỏng nhẹ", "Hư hỏng nặng", "Mất sách"], width=15)
        ...
        def process_return():
            try:
                ...
                if int(so_luong) > remaining:
                    messagebox.showwarning("Cảnh báo", f"Số lượng trả không được vượt quá số lượng còn lại ({remaining})!")
                    return
        ...
        # Return buttons
        btn_return = tk.Button(button_frame_return, text="Trả sách", bg="#1abc9c", fg="white", font=("Arial", 10), width=10, command=process_return)
        btn_return.pack(side="left", padx=5)

        btn_refresh = tk.Button(button_frame_return, text="Làm mới", bg="#95a5a6", fg="white", font=("Arial", 10), width=10, command=refresh_active_borrows)
        btn_refresh.pack(side="left", padx=5)

        # Print return slip button
        ...
        columns_return = ["Mã Phiếu", "Mã Độc Giả", "Tên Độc Giả", "Mã Sách", "Tên Sách", "Số Lượng", "Đã Trả", "Còn Lại", "Ngày Mượn", "Ngày Hẹn Trả"]
        ...
        return_history_columns = ["Mã Phiếu Trả", "Mã Phiếu Mượn", "Mã Độc Giả", "Tên Độc Giả", "Ngày Trả", "Ngày Mượn", "Ngày Hẹn Trả", "Quá Hạn"]
        return_history_list = ttk.Treeview(return_history_frame, columns=return_history_columns, show="headings", height=10)
        ...
        history_x_scrollbar = ttk.Scrollbar(return_history_frame, orient="horizontal", command=return_history_list.xview)
        ...
        def show_return_history_details(event):
            ...
            if selected_items:
                ...
                # Add print button
                btn_print = tk.Button(details_window, text="In phiếu", command=lambda: self.print_return_slip(ma_phieu_tra),
    ...
    def show_users_management(self):
        ...
        for i, field_text in enumerate(fields):
            ...
            if field_text == "Giới Tính":
                ...
                entry = ttk.Combobox(field_frame, values=["Giảng viên", "Sinh viên", "Admin", "Thủ Thư"], width=25)
                entry.pack(side="left", fill="x", expand=True)
        ...
        def add_user():
            try:
                ...
                if self.user_manager.add_user(ma_doc_gia, ho_ten, gioi_tinh, ngay_sinh, dien_thoai, mat_khau, phan_quyen):
        ...
        def delete_user():
            ...
            if messagebox.askyesno("Xác nhận", f"Bạn có chắc chắn muốn xóa người dùng '{self.selected_user[1]}' không?"):
                try:
        ...
        # Function buttons
        btn_add = tk.Button(button_frame, text="Thêm", bg="#1abc9c", fg="white", font=("Arial", 10), width=10, command=add_user)
        btn_add.pack(side="left", padx=5)

        btn_edit = tk.Button(button_frame, text="Sửa", bg="#f39c12", fg="white", font=("Arial", 10), width=10, command=edit_user)
        btn_edit.pack(side="left", padx=5)

        btn_delete = tk.Button(button_frame, text="Xóa", bg="#e74c3c", fg="white", font=("Arial", 10), width=10, command=delete_user)
        btn_delete.pack(side="left", padx=5)

        btn_search = tk.Button(button_frame, text="Tìm kiếm", bg="#3498db", fg="white", font=("Arial", 10), width=10, command=search_user)
        btn_search.pack(side="left", padx=5)
        ...
        for col in columns:
            ...
            elif col in ["Phân Quyền", "Ngày Sinh", "Điện Thoại"]:
                user_list.column(col, width=90, anchor="center")
            elif col in ["Giới Tính"]:
                user_list.column(col, width=90, anchor="center")
            elif col == "Họ và Tên":
                user_list.column(col, width=120, anchor="center")
            else:
    ...
    def show_reports(self):
        ...
        # Create overdue books table - Tạo trong overdue_frame_table thay vì overdue_frame
        columns = ["Mã Phiếu", "Mã Độc Giả", "Tên Độc Giả", "Mã Sách", "Tên Sách", "Số Lượng", "Ngày Mượn",
                "Ngày Hẹn Trả", "Số Ngày Quá Hạn"]
        overdue_list = ttk.Treeview(overdue_frame_table, columns=columns, show="headings", height=15)
        ...
        # Thêm thanh cuộn - đặt trong cùng frame với Treeview
        ...
        # Sắp xếp các thành phần - đảm bảo thứ tự đúng
        overdue_list.pack(side="left", fill="both", expand=True)
        ...
        try:
            ...
            print(f"Số lượng sách quá hạn: {len(overdue_books) if overdue_books else 0}")
            ...
            if overdue_books:
                for book in overdue_books:
                    overdue_list.insert("", "end", values=(
                        ...
                        book[3],  # NgayMuon
                        book[4],  # NgayHenTra
                        f"{int(book[9])} ngày" if book[9] and book[9] > 0 else "0 ngày"
                    ))
            else:
                tk.Label(overdue_frame, text="Không có sách quá hạn", font=("Arial", 11), bg="#f8f9fa").pack(pady=10)
        except Exception as e:
            print(f"Lỗi khi tải dữ liệu sách quá hạn: {str(e)}")
            tk.Label(overdue_frame, text=f"Lỗi: {str(e)}", font=("Arial", 11), fg="red", bg="#f8f9fa").pack(pady=10)


        btn_export_pdf = tk.Button(self.content_frame, text="Xuất báo cáo", bg="#3498db", fg="white",
                                   font=("Arial", 10), width=20,
                                   command=self.export_report_word)
